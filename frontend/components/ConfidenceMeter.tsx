...
function ScoreRing({ score, celebrated }: { score: number; celebrated?: boolean }) {
  ...
  const ringColor = score >= 80 ? "#16a34a" : score >= 60 ? "#d97706" : "#5B4CE8";
    ...
    <div className="flex justify-center relative">
      ...
      >
        ...
        <text x="50" y="60" textAnchor="middle" fontSize="9"
          fill="currentColor" className="fill-gray-400">
          confident
        </text>
      </motion.svg>

      {/* "Ready" badge when celebrated */}
      <AnimatePresence>
        {celebrated && (
          <motion.div
            ...
            className="absolute -bottom-1 left-1/2 -translate-x-1/2 text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300 whitespace-nowrap"
          >
            ✓ Ready to recommend
          </motion.div>
        )}
  ...
  return (
    <div className="py-1.5 border-b border-gray-100 dark:border-gray-800 last:border-0">
      <div className="flex justify-between items-center mb-0.5">
        ...
        <span className={`text-xs font-medium ${isNegative ? "text-amber-600" : "text-gray-700 dark:text-gray-300"}`}>
...
export default function ConfidenceMeter({ score, breakdown, journey, celebrated }: Props) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <ScoreRing score={score} celebrated={celebrated} />
      </div>
          ...
          <div className="flex flex-col gap-2">
            {journey.map((step, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500 flex-1 truncate">{step.label}</span>
                <span className="text-xs font-medium text-gray-600 dark:text-gray-400">{step.score}%</span>
              </div>
            ))}
          </div>
        </div>
